...
from CustomVIT import vit_b_16_faster
import random
import numpy as np
...
def test(epoch, max_epochs, net, patch_producer, testloader, criterion, device):
    ...
    return balanced_accuracy_score(all_targets, all_preds),all_preds,all_targets
...
def fit_model(model, patch_producer, trainloader, testloader, device, epochs:int, learning_rate:float, lr_p, max_lr:float, momentum:float, save_path:str, bias=0.1, cosine=False):
    ...
    for epoch in range(epochs):
        ...
        acc, all_p, all_t  = test(epoch, epochs, model, patch_producer, testloader, criterion, device)
        if acc > best_acc:
            if best_name != "":
                os.remove(best_name)
            best_acc = acc
    ...
    f.close()
    return best_name, best_acc, best_p, best_t
...
def main(dataset:str, model_name:str, epochs:int, learning_rate:float, lr_p, batch_size:int, max_lr:float, momentum:float, output_prefix:str, cosine:bool):
    ...
    best_name, best_accuracy, best_preda, best_targeta = fit_model(model, patch_producer, trainloader, testloader, device, epochs, learning_rate, lr_p, max_lr, momentum, "trained_models/" + model_name + "/" + output_prefix + dataset + "_" + model_name, bias, cosine)
    print("Training complete: " + best_name + " with accuracy: " + str(round(best_accuracy, 4)))
    return str(round(best_accuracy, 4)), best_preda, best_targeta
        ...
        for lr_p in learning_rates_p:
            for momentum in momentums:
                for cosine in cosines:
                    ...
                    accuracy, best_preds, best_targets= main(args.dataset, args.model, args.epochs, lr, lr_p, args.batch_size, args.max_lr, momentum, tag, cosine)
